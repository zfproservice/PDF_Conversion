...
def clean_and_fill_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Replace single or double quote ditto marks with None
    df = df.replace(to_replace=r'^\s*["\u201c\u201d\u201e\u201f\u2033\u2036]\s*$', value=None, regex=True)
    # Exclude event rows like ON LEAVE from auto-fill if needed, or forward fill across the rest
    df = df.ffill()
    return df
...
def df_to_excel(df: pd.DataFrame) -> bytes:
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # Access openpyxl worksheet for basic formatting
    ...
    if st.button("Convert to Excel", type="primary"):
        with st.spinner("Analyzing PDF with Gemini..."):
            try:
                ...
                # Send request to Gemini 2.5 Flash (Free Tier)
